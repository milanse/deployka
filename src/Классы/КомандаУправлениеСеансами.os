...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, 
		"-lockend",
		"Окончания блокировки пользователей, время указываем как '2040-12-31T23:59:59'");
	...
	Парсер.ДобавитьПараметрФлагКоманды(ОписаниеКоманды,
		...
		"Включить управление блокировкой регламентных заданий");
...
Процедура ПрочитатьПараметры(Знач ПараметрыКоманды)
	...
	мНастройки.Вставить("ВремяОкончанияБлокировки", ПараметрыКоманды["-lockend"]);
	...
	мНастройки.Вставить("УправлятьРегламентнымиЗаданиями", ПараметрыКоманды["-include-scheduled-jobs"]);
...
Процедура УстановитьСтатусБлокировкиСеансов(Знач Блокировать)
	...
	Если мНастройки.УправлятьРегламентнымиЗаданиями Тогда
		стрБлокироватьРегламентныеЗадания = "--scheduled-jobs-deny="+?(Блокировать, "on", "off");
	Иначе
		стрБлокироватьРегламентныеЗадания = "";
	КонецЕсли; 
	
	КомандаВыполнения = СтрокаЗапускаКлиента() + СтрШаблон("infobase update --infobase=""%3""%4 --cluster=""%1""%2 %10 --sessions-deny=%5 --denied-message=""%6"" --denied-from=""%8"" --denied-to=""%9"" --permission-code=""%7""",
		...
		ВремяБлокировки,
		мНастройки.ВремяОкончанияБлокировки,
		стрБлокироватьРегламентныеЗадания) + " "+мНастройки.АдресСервераАдминистрирования;
